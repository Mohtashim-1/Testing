...
class EarlyCompensation(Document):
	@frappe.whitelist()
	def get_data(self):
		rec = frappe.db.sql("""
			...
			WHERE p.month=%s and p.year=%s and p.employee=%s and c.early_ot is not null and c.check_in_1 is not null and c.approved_eot is null
