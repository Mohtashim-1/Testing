...
class LateOverTime(Document):
	...
	def get_data(self):
		...
		rec = frappe.db.sql("""
			select p.employee, p.employee_name, p.designation, c.late_sitting, c.approved_ot1, c.name as child_name, p.name as parent_name 
			from `tabEmployee Attendance` p
			LEFT JOIN `tabEmployee Attendance Table` c ON c.parent=p.name
			where c.date=%s and c.late_sitting > %s and (c.approved_ot1 = '' or c.approved_ot1 is null) 
		""", (self.date, self.ot_frequency), as_dict=1)  # Only pass self.date, not self.ot_frequency
